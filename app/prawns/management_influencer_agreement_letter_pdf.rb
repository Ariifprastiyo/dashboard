...
class ManagementInfluencerAgreementLetterPdf < Prawn::Document
  def initialize(media_plan, scope_of_work, influencer, view, total, total_influencer)
    super(top_margin: 70, page_size: "A4")

    @media_plan = media_plan
    @scope_of_work = scope_of_work
    @management = @scope_of_work.management
  ...
  def first_party
    ...
    table(data,
      width: 500,
      column_widths: { 0 => 150 },
      cell_style: { size: 12, inline_format: true, align: :left, valign: :left, border_width: 0 }
    )
  ...
  def second_party
    ...
    data << ["Nama", ": #{@management.pic_name}"]
    data << ["Alamat", ": #{@management.address}"]
    data << ["NIK", ": #{@management.no_ktp}"]
    data << ["NPWP", ": #{@management.no_npwp}"]
    data << ["Nomor Handphone", ": #{@management.phone}"]
    data << ["Email", ": #{@management.pic_email}"]
    ...
    move_down 10
    text "Yang bertindak sebagai <b>Influencer Management (<i>#{@scope_of_work.management.name}</i>)</b> atas Campaign <b>#{@media_plan.campaign.name}</b> yang selanjutnya disebut sebagai <b>PIHAK KEDUA</b>.", inline_format: true
    move_down 5
  ...
  def chapter1
    ...
    data << ["4.", "Membayar kompensasi kerjasama sebesar <b>IDR #{@view.number_with_delimiter(@total)}</b> untuk total <b>#{@total_influencer} influencer</b> di bawah naungan PIHAK KEDUA"]
    ...
    if @scope_of_work.agreement_payment_terms_note.blank?
      sentences << ["b.", "Pembayaran pelunasan akan dilakukan pada H+7 <i>(Full Payment)</i> setelah PIHAK KEDUA menyelesaikan pekerjaan dan mengirimkan invoice kepada PIHAK PERTAMA."]
    else
  ...
  def chapter2
    ...
    data << ["3.", "Seluruh Influencer di bawah naungan pihak wajib mempublikasikan postingan <i>campaign</i> di social media influencer dengan pembagian sebagai berikut:"]
  ...
  def chapter4
    ...
    data1 << ["3.", "Masa kontrak ini pada tanggal <b>#{@scope_of_work.agreement_end_date || (Date.today + 7.days).strftime("%d %B %Y")}</b>. Bilamana terdapat perubahan periode campaign, maka PIHAK PERTAMA <b>berkewajiban</b> menginformasikan kepada PIHAK KEDUA untuk kemudian mendapatkan persetujuan."]
